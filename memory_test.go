...
func TestMemoryStore_RegenerateID(t *testing.T) {
	f := flamego.NewWithLogger(&bytes.Buffer{})
	f.Use(Sessioner())

	f.Get("/set", func(w http.ResponseWriter, r *http.Request, s Session) {
		require.Nil(t, s.RegenerateID(w, r))
		s.Set("username", "flamego")
	})
	f.Get("/get", func(s Session) {
		username, ok := s.Get("username").(string)
	...
	// Use the regenerated session cookie (the last Set-Cookie of the response).
...
func TestMemoryStore_RegenerateIDOnly(t *testing.T) {
	f := flamego.NewWithLogger(&bytes.Buffer{})
	f.Use(Sessioner())
	...
	f.Get("/rotate", func(w http.ResponseWriter, r *http.Request, s Session) {
		oldSid = s.ID()
		require.Nil(t, s.RegenerateID(w, r))
	})
	f.Get("/get", func(c flamego.Context, s Session, store Store) {
		// The old session ID must no longer be indexed after the rotation.
		assert.False(t, store.Exist(c.Request().Context(), oldSid))
	...
	// Regenerate the session ID without touching any session data.
	resp = httptest.NewRecorder()
	req, err = http.NewRequest(http.MethodGet, "/rotate", nil)
	require.Nil(t, err)
	req.Header.Set("Cookie", cookie)
	f.ServeHTTP(resp, req)
	assert.Equal(t, http.StatusOK, resp.Code)
